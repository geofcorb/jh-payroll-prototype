...
planning. Audience is the employee, not the project team.
...
## Running
Open `payroll-compare/index.html` or `employee-paycheck/index.html` in any browser.
Or use the preview servers defined in `.claude/launch.json` (each is just
`python3 -m http.server` rooted at the app folder):
